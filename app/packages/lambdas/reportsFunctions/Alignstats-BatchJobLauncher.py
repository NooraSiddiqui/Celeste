...
def lambda_handler(event, context):
    ...
    if isAlignment(key):  # only generate reports for alignment files e.g. bam
        ...
        command = [
            ...
            inputfile,
            "-m",
            "s3://hgsccl-op-data/alignstats/masks/hs37d5_N_regions_autosome.bed",
            "-o",
            out,
            "-C",
            "-r",
            "s3://hgsccl-op-data/alignstats/regions/hs37d5_autosome.bed",
            "-P",
            "1",
            "-F",
            "2048",
        ...
        if inputfile.endswith(".cram"):
            command.extend(["-T", "s3://hgsccl-op-data/bwa_references/h/hs37d5/hs37d5.fa"])
        try:
